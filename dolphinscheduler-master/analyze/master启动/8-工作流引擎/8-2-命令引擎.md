        ...
        alt 命令列表为空
            ...
            par 并发处理每个命令
                ...
                alt 删除失败(已被其他Master处理)
                    ...
                    Note over WEFW,WSA: 阶段3: 事件触发和处理
                    Note over WEFW: 定期触发(100ms间隔)
                    WEFW->>WEB: poll() 获取事件
...
```mermaid
flowchart TD
    Start([CommandEngine主循环开始]) --> CheckLoad{检查系统负载}
    ...
    DeleteResult -->|成功| FindHandler[根据commandType查找Handler]
    ...
    EventEnqueue --> WorkerTrigger[Worker定期触发<br/>100ms间隔]
    
    WorkerTrigger --> PollEvent[从DelayQueue取出事件]
    PollEvent --> EventEmpty{事件队列是否为空?}
    EventEmpty -->|是| WorkerTrigger
    EventEmpty -->|否| FindEventHandler[根据eventType查找EventHandler]
    
    FindEventHandler --> EventHandlerExists{EventHandler是否存在?}
