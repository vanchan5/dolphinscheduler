...
以下流程图详细展示了任务从创建到结束的完整状态流转过程，包括触发状态转换的事件和条件：

```mermaid
stateDiagram-v2
    [*] --> SUBMITTED_SUCCESS: TaskStartLifecycleEvent<br/>初始化任务实例
    
    state SUBMITTED_SUCCESS {
        [*] --> 检查工作流状态
        检查工作流状态 --> 准备暂停: isWorkflowReadyPause()
        检查工作流状态 --> 准备停止: isWorkflowReadyStop()
        检查工作流状态 --> 检查任务组: 正常流程
        检查任务组 --> 需要任务组: isTaskNeedAcquireTaskGroupSlot()
        检查任务组 --> 发布分发事件: 无需任务组
        需要任务组 --> 等待任务组: acquireTaskGroupSlot()
        等待任务组 --> 发布分发事件: TaskDispatchLifecycleEvent(唤醒)
    }
    
    SUBMITTED_SUCCESS --> DELAY_EXECUTION: TaskDispatchLifecycleEvent<br/>delayTime > 0
    SUBMITTED_SUCCESS --> DISPATCH: TaskDispatchLifecycleEvent<br/>delayTime = 0<br/>加入分发队列
    
    state DELAY_EXECUTION {
        [*] --> 延迟等待
        延迟等待 --> 延迟到期: 延迟时间到期
        延迟到期 --> 加入队列: dispatchTaskExecuteRunnableWithDelay()
    }
    
    DELAY_EXECUTION --> DISPATCH: GlobalTaskDispatchWaitingQueueLooper<br/>takeTaskExecuteRunnable()
    
    state DISPATCH {
        [*] --> 选择执行器
        选择执行器 --> 逻辑任务: isLogicTask()
        选择执行器 --> 物理任务: 非逻辑任务
        逻辑任务 --> 调用Master: LogicTaskExecutorClientDelegator
        物理任务 --> 选择Worker: WorkerLoadBalancer.select()
        选择Worker --> 调用Worker: PhysicalTaskExecutorClientDelegator
        调用Master --> 分发成功: TaskExecutorDispatchResponse
        调用Worker --> 分发成功: TaskExecutorDispatchResponse
        分发成功 --> 更新状态: TaskDispatchedLifecycleEvent
    }
    
    DISPATCH --> RUNNING_EXECUTION: TaskDispatchedLifecycleEvent<br/>dispatchedEventAction()<br/>state = DISPATCH
    DISPATCH --> SUBMITTED_SUCCESS: 分发失败<br/>重新入队(重试)
    
    state RUNNING_EXECUTION {
        [*] --> Worker执行中
        Worker执行中 --> 接收状态更新: TaskRuntimeContextChangedEvent
        Worker执行中 --> 执行完成: 任务执行完成
    }
    
    RUNNING_EXECUTION --> SUCCESS: TaskRunningLifecycleEvent<br/>startedEventAction()<br/>TaskSuccessLifecycleEvent<br/>succeedEventAction()
    RUNNING_EXECUTION --> FAILURE: TaskFailedLifecycleEvent<br/>failedEventAction()
    RUNNING_EXECUTION --> KILL: TaskKillLifecycleEvent<br/>killEventAction()<br/>TaskKilledLifecycleEvent<br/>killedEventAction()
    RUNNING_EXECUTION --> PAUSE: TaskPauseLifecycleEvent<br/>pauseEventAction()<br/>TaskPausedLifecycleEvent<br/>pausedEventAction()
    
    state FAILURE {
        [*] --> 检查重试
        检查重试 --> 可以重试: isTaskInstanceCanRetry()
        检查重试 --> 不能重试: !isTaskInstanceCanRetry()
        可以重试 --> 发布重试事件: TaskRetryLifecycleEvent
        不能重试 --> 标记失败: markTaskExecutionRunnableChainFailure()
    }
    
    FAILURE --> SUBMITTED_SUCCESS: TaskRetryLifecycleEvent<br/>retryEventAction()<br/>创建重试任务实例
    ...
    PAUSE --> RUNNING_EXECUTION: 工作流恢复<br/>TaskStartLifecycleEvent
    ...
    RUNNING_EXECUTION --> FORCED_SUCCESS: 手动强制成功<br/>TaskSuccessLifecycleEvent<br/>succeedEventAction()
    
    note right of SUBMITTED_SUCCESS
        事件: TaskStartLifecycleEvent
        Handler: TaskStartLifecycleEventHandler
        StateAction: TaskSubmittedStateAction
        动作: startEventAction()
    end note
    
    note right of DISPATCH
        事件: TaskDispatchLifecycleEvent
        Handler: TaskDispatchLifecycleEventHandler
        StateAction: TaskSubmittedStateAction
        动作: dispatchEventAction()
        
        事件: TaskDispatchedLifecycleEvent
        Handler: TaskDispatchedLifecycleEventHandler
        StateAction: TaskDispatchStateAction
        动作: dispatchedEventAction()
    end note
    
    note right of RUNNING_EXECUTION
        事件: TaskRunningLifecycleEvent
        Handler: TaskRunningLifecycleEventHandler
        StateAction: TaskRunningStateAction
        动作: startedEventAction()
        
        事件: TaskSuccessLifecycleEvent
        Handler: TaskSuccessLifecycleEventHandler
        StateAction: TaskSuccessStateAction
        动作: succeedEventAction()
        
        事件: TaskFailedLifecycleEvent
        Handler: TaskFailedLifecycleEventHandler
        StateAction: TaskFailureStateAction
        动作: failedEventAction()
    end note
```

**状态流转说明**:

1. **初始状态 (SUBMITTED_SUCCESS)**:
   - **触发事件**: `TaskStartLifecycleEvent`
   - **处理流程**: 
     - 检查工作流状态（暂停/停止）
     - 检查是否需要任务组资源
     - 发布 `TaskDispatchLifecycleEvent` 进行分发

2. **延迟执行 (DELAY_EXECUTION)**:
   - **触发条件**: 任务配置了延迟时间 (`delayTime > 0`)
   - **转换**: 延迟时间到期后，通过队列处理器取出，进入 `DISPATCH` 状态

3. **分发中 (DISPATCH)**:
   - **触发事件**: `TaskDispatchLifecycleEvent`
   - **处理流程**:
     - 判断任务类型（逻辑任务/物理任务）
     - 逻辑任务：调用Master服务
     - 物理任务：选择Worker并调用Worker服务
   - **成功转换**: `TaskDispatchedLifecycleEvent` → `RUNNING_EXECUTION`
   - **失败处理**: 重新入队，状态回到 `SUBMITTED_SUCCESS`

4. **运行中 (RUNNING_EXECUTION)**:
   - **触发事件**: `TaskRunningLifecycleEvent` (Worker开始执行)
   - **可能转换**:
     - `TaskSuccessLifecycleEvent` → `SUCCESS`
     - `TaskFailedLifecycleEvent` → `FAILURE`
     - `TaskKillLifecycleEvent` → `KILL`
     - `TaskPauseLifecycleEvent` → `PAUSE`

5. **失败状态 (FAILURE)**:
   - **重试机制**: 如果任务可重试，发布 `TaskRetryLifecycleEvent`，创建新的任务实例，回到 `SUBMITTED_SUCCESS`
   - **最终失败**: 如果不可重试，标记任务链失败

6. **暂停状态 (PAUSE)**:
   - **恢复**: 工作流恢复后，可以重新发布 `TaskStartLifecycleEvent`，回到 `RUNNING_EXECUTION`

7. **结束状态**:
   - `SUCCESS`: 任务执行成功
   - `KILL`: 任务被杀死
   - `FORCED_SUCCESS`: 任务被强制成功

**关键事件触发点**:

| 状态转换 | 触发事件 | Handler | StateAction方法 |
|---------|---------|---------|----------------|
| `[*]` → `SUBMITTED_SUCCESS` | `TaskStartLifecycleEvent` | `TaskStartLifecycleEventHandler` | `startEventAction()` |
| `SUBMITTED_SUCCESS` → `DISPATCH` | `TaskDispatchLifecycleEvent` | `TaskDispatchLifecycleEventHandler` | `dispatchEventAction()` |
| `DISPATCH` → `RUNNING_EXECUTION` | `TaskDispatchedLifecycleEvent` | `TaskDispatchedLifecycleEventHandler` | `dispatchedEventAction()` |
| `DISPATCH` → `RUNNING_EXECUTION` | `TaskRunningLifecycleEvent` | `TaskRunningLifecycleEventHandler` | `startedEventAction()` |
| `RUNNING_EXECUTION` → `SUCCESS` | `TaskSuccessLifecycleEvent` | `TaskSuccessLifecycleEventHandler` | `succeedEventAction()` |
| `RUNNING_EXECUTION` → `FAILURE` | `TaskFailedLifecycleEvent` | `TaskFailedLifecycleEventHandler` | `failedEventAction()` |
| `FAILURE` → `SUBMITTED_SUCCESS` | `TaskRetryLifecycleEvent` | `TaskRetryLifecycleEventHandler` | `retryEventAction()` |
| `RUNNING_EXECUTION` → `KILL` | `TaskKillLifecycleEvent` | `TaskKillLifecycleEventHandler` | `killEventAction()` |
| `RUNNING_EXECUTION` → `PAUSE` | `TaskPauseLifecycleEvent` | `TaskPauseLifecycleEventHandler` | `pauseEventAction()` |
...
stateDiagram-v2
    ...
    FAILURE --> SUBMITTED_SUCCESS: 可重试，创建重试任务
    PAUSE --> RUNNING_EXECUTION: 工作流恢复
