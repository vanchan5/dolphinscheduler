...
4. **阶段4：事件触发和处理**
   ...
   - Handler根据工作流状态从WorkflowStateActionFactory获取对应的WorkflowStateAction
   - WorkflowStateAction执行具体的业务逻辑（如触发任务、状态转换等）
   - **状态映射关系**：根据`WorkflowExecutionStatus`选择对应的`IWorkflowStateAction`实现
     - `SUBMITTED_SUCCESS` → `WorkflowSubmittedStateAction`
     - `RUNNING_EXECUTION` → `WorkflowRunningStateAction`
     - `READY_PAUSE` → `WorkflowReadyPauseStateAction`
     - `PAUSE` → `WorkflowPausedStateAction`
     - `READY_STOP` → `WorkflowReadyStopStateAction`
     - `STOP` → `WorkflowStoppedStateAction`
     - `FAILURE` → `WorkflowFailedStateAction`
     - `SUCCESS` → `WorkflowSuccessStateAction`
     - `SERIAL_WAIT` → `WorkflowSerialWaitStateAction`
     - `FAILOVER` → `WorkflowFailoverStateAction`
...
flowchart TD
    ...
    FEH --> GetState
    FEH2 --> GetState
    GetState --> SelectStateAction{根据状态选择WorkflowStateAction}
    SelectStateAction -->|SUBMITTED_SUCCESS| WSSA[WorkflowSubmittedStateAction]
    SelectStateAction -->|RUNNING_EXECUTION| WRSA[WorkflowRunningStateAction]
    SelectStateAction -->|READY_PAUSE| WRPSA[WorkflowReadyPauseStateAction]
    SelectStateAction -->|PAUSE| WPSA[WorkflowPausedStateAction]
    SelectStateAction -->|READY_STOP| WRSSA[WorkflowReadyStopStateAction]
    SelectStateAction -->|STOP| WSTSA[WorkflowStoppedStateAction]
    SelectStateAction -->|FAILURE| WFSA[WorkflowFailedStateAction]
    SelectStateAction -->|SUCCESS| WSCSA[WorkflowSuccessStateAction]
    SelectStateAction -->|SERIAL_WAIT| WSESA[WorkflowSerialWaitStateAction]
    SelectStateAction -->|FAILOVER| WFOSA[WorkflowFailoverStateAction]
    WSSA --> GetStateAction[从WorkflowStateActionFactory获取WorkflowStateAction]
    WRSA --> GetStateAction
    WRPSA --> GetStateAction
    WPSA --> GetStateAction
    WRSSA --> GetStateAction
    WSTSA --> GetStateAction
    WFSA --> GetStateAction
    WSCSA --> GetStateAction
    WSESA --> GetStateAction
    WFOSA --> GetStateAction
    GetStateAction --> LogBegin[记录开始日志]
    LogBegin --> CallStateAction[调用WorkflowStateAction的相应方法]
    ...
    subgraph EventStateMapping[WorkflowLifecycleEventType ↔ WorkflowExecutionStatus 对应关系]
        direction LR
        
        subgraph Events[事件类型 WorkflowLifecycleEventType]
            direction TB
            ET1[START]
            ET2[TOPOLOGY_LOGICAL_TRANSACTION_WITH_TASK_FINISH]
            ET3[PAUSE]
            ET4[PAUSED]
            ET5[STOP]
            ET6[STOPPED]
        ...
        subgraph StateTransitions[状态转换 WorkflowExecutionStatus]
            direction TB
            ST1[RUNNING_EXECUTION<br/>保持或触发任务]
            ST2[保持当前状态<br/>触发后继任务]
            ST3[READY_PAUSE<br/>准备暂停]
            ST4[PAUSE<br/>已暂停]
            ST5[READY_STOP<br/>准备停止]
            ST6[STOP<br/>已停止]
            ST7[SUCCESS<br/>成功]
            ST8[FAILURE<br/>失败]
            ST9[发布FINALIZE事件<br/>最终完成]
        end
        
        ET1 -->|startEventAction| ST1
        ET2 -->|topologyLogicalTransitionEventAction| ST2
        ET3 -->|pauseEventAction<br/>RUNNING_EXECUTION→| ST3
        ET4 -->|pausedEventAction<br/>READY_PAUSE→| ST4
        ET5 -->|stopEventAction<br/>RUNNING_EXECUTION→| ST5
        ET6 -->|stoppedEventAction<br/>READY_STOP→| ST6
        ET7 -->|succeedEventAction<br/>任何状态→| ST7
        ET8 -->|failedEventAction<br/>任何状态→| ST8
        ET9 -->|finalizeEventAction| ST9
    end
    ...
    style SelectStateAction fill:#e3f2fd
    style WSSA fill:#e8f5e9
    style WRSA fill:#e8f5e9
    style WRPSA fill:#e8f5e9
    style WPSA fill:#e8f5e9
    style WRSSA fill:#e8f5e9
    style WSTSA fill:#e8f5e9
    style WFSA fill:#e8f5e9
    style WSCSA fill:#e8f5e9
    style WSESA fill:#e8f5e9
    style WFOSA fill:#e8f5e9
    style GetStateAction fill:#fff3e0
