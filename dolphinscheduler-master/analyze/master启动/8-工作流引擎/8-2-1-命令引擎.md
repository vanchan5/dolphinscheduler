...
EventType -->|TOPOLOGY_LOGICAL_TRANSACTION_WITH_TASK_FINISH| TLEH[WorkflowTopologyLogicalTransitionWithTaskFinishLifecycleEventHandler<br/>拓扑逻辑转换事件处理器]
...
WSLEH --> GetState[获取工作流当前状态<br/>workflowExecutionRunnable.getState<br/>返回WorkflowExecutionStatus]
TLEH --> GetState
PLEH --> GetState
PLDEH --> GetState
SLEH --> GetState
...
GetState --> GetWorkflowStateAction[根据WorkflowExecutionStatus<br/>从WorkflowStateActionFactory获取IWorkflowStateAction<br/>workflowStateActionFactory.getAction-status]

GetWorkflowStateAction --> StateMapping{WorkflowStateActionFactory<br/>根据状态返回对应的实现类}
StateMapping -->|SUBMITTED_SUCCESS_0| SA1[WorkflowSubmittedStateAction]
StateMapping -->|RUNNING_EXECUTION_1| SA2[WorkflowRunningStateAction]
StateMapping -->|READY_PAUSE_2| SA3[WorkflowReadyPauseStateAction]
StateMapping -->|PAUSE_3| SA4[WorkflowPausedStateAction]
StateMapping -->|READY_STOP_4| SA5[WorkflowReadyStopStateAction]
StateMapping -->|STOP_5| SA6[WorkflowStoppedStateAction]
StateMapping -->|FAILURE_6| SA7[WorkflowFailedStateAction]
StateMapping -->|SUCCESS_7| SA8[WorkflowSuccessStateAction]
StateMapping -->|SERIAL_WAIT_14| SA9[WorkflowSerialWaitStateAction]
StateMapping -->|FAILOVER_18| SA10[WorkflowFailoverStateAction]

SA1 --> IWorkflowStateAction[IWorkflowStateAction实例]
SA2 --> IWorkflowStateAction
SA3 --> IWorkflowStateAction
SA4 --> IWorkflowStateAction
SA5 --> IWorkflowStateAction
SA6 --> IWorkflowStateAction
SA7 --> IWorkflowStateAction
SA8 --> IWorkflowStateAction
SA9 --> IWorkflowStateAction
SA10 --> IWorkflowStateAction

IWorkflowStateAction --> LogBegin[记录开始日志]
LogBegin --> CallWorkflowStateAction[调用WorkflowStateAction的相应方法]

CallWorkflowStateAction --> ActionType{根据WorkflowLifecycleEventType<br/>选择对应方法}
ActionType -->|START| StartAction[startEventAction<br/>触发开始节点]
ActionType -->|TOPOLOGY_LOGICAL_TRANSACTION_WITH_TASK_FINISH| TopologyAction[topologyLogicalTransitionEventAction<br/>触发后继任务]
ActionType -->|PAUSE| PauseAction[pauseEventAction<br/>暂停任务]
ActionType -->|PAUSED| PausedAction[pausedEventAction<br/>处理已暂停状态]
ActionType -->|STOP| StopAction[stopEventAction<br/>停止任务]
ActionType -->|STOPPED| StoppedAction[stoppedEventAction<br/>处理已停止状态]
ActionType -->|SUCCEED| SucceedAction[succeedEventAction<br/>完成工作流]
ActionType -->|FAILED| FailedAction[failedEventAction<br/>完成工作流]
ActionType -->|FINALIZE| FinalizeAction[finalizeEventAction<br/>完成工作流清理]

StartAction --> TriggerTasks[triggerTasks触发任务]
TopologyAction --> TriggerSuccessors[触发后继任务]
PauseAction --> PauseTasks[暂停活跃任务]
PausedAction --> PausedProcess[处理已暂停状态]
StopAction --> KillTasks[杀死活跃任务]
StoppedAction --> StoppedProcess[处理已停止状态]
SucceedAction --> WorkflowFinish1[workflowFinish完成工作流]
FailedAction --> WorkflowFinish2[workflowFinish完成工作流]
FinalizeAction --> FinalizeProcess[完成工作流清理]

TriggerTasks --> PublishTaskEvent[发布TaskStartLifecycleEvent]
TriggerSuccessors --> PublishTaskEvent
PauseTasks --> UpdateState[更新工作流状态]
PausedProcess --> UpdateState
KillTasks --> UpdateState
StoppedProcess --> UpdateState
WorkflowFinish1 --> UpdateState
WorkflowFinish2 --> UpdateState
FinalizeProcess --> UpdateState
...
style GetState fill:#e3f2fd
style GetWorkflowStateAction fill:#fff3e0
style StateMapping fill:#e8f5e9
style IWorkflowStateAction fill:#f1f8e9
style SA1 fill:#e8f5e9
style SA2 fill:#e8f5e9
style SA3 fill:#e8f5e9
style SA4 fill:#e8f5e9
style SA5 fill:#e8f5e9
style SA6 fill:#e8f5e9
style SA7 fill:#e8f5e9
style SA8 fill:#e8f5e9
style SA9 fill:#e8f5e9
style SA10 fill:#e8f5e9
style CallWorkflowStateAction fill:#fff9c4
style ActionType fill:#fce4ec
style PausedAction fill:#f1f8e9
style StoppedAction fill:#f1f8e9
style FinalizeAction fill:#f1f8e9
```
...
工作流状态流转由 `IWorkflowStateAction` 的各种事件处理方法触发，通过 `transformWorkflowInstanceState()` 方法实现状态转换。以下是详细的状态流转路径：

**状态流转图**：

```mermaid
stateDiagram-v2
    [*] --> SUBMITTED_SUCCESS: CommandHandler.assembleWorkflowInstance()
    
    SUBMITTED_SUCCESS --> RUNNING_EXECUTION: CommandHandler设置状态
    
    RUNNING_EXECUTION --> READY_PAUSE: pauseEventAction()<br/>transformWorkflowInstanceState(READY_PAUSE)
    RUNNING_EXECUTION --> READY_STOP: stopEventAction()<br/>transformWorkflowInstanceState(READY_STOP)
    RUNNING_EXECUTION --> SUCCESS: succeedEventAction()<br/>workflowFinish(SUCCESS)
    RUNNING_EXECUTION --> FAILURE: failedEventAction()<br/>workflowFinish(FAILURE)
    
    READY_PAUSE --> PAUSE: pausedEventAction()<br/>workflowFinish(PAUSE)
    READY_PAUSE --> SUCCESS: succeedEventAction()<br/>workflowFinish(SUCCESS)
    READY_PAUSE --> FAILURE: failedEventAction()<br/>workflowFinish(FAILURE)
    
    READY_STOP --> STOP: stoppedEventAction()<br/>workflowFinish(STOP)
    READY_STOP --> SUCCESS: succeedEventAction()<br/>workflowFinish(SUCCESS)
    READY_STOP --> FAILURE: failedEventAction()<br/>workflowFinish(FAILURE)
    
    PAUSE --> [*]: finalizeEventAction()<br/>清理工作流
    STOP --> [*]: finalizeEventAction()<br/>清理工作流
    SUCCESS --> [*]: finalizeEventAction()<br/>清理工作流
    FAILURE --> [*]: finalizeEventAction()<br/>清理工作流
    
    note right of RUNNING_EXECUTION
        主要执行状态
        - startEventAction(): 触发任务
        - topologyLogicalTransitionEventAction(): 触发后继任务
        - emitWorkflowFinishedEventIfApplicable(): 
          自动检测并发布完成事件
    end note
    
    note right of READY_PAUSE
        准备暂停状态
        - 等待所有任务暂停完成
        - pausedEventAction()触发后转为PAUSE
    end note
    
    note right of READY_STOP
        准备停止状态
        - 等待所有任务停止完成
        - stoppedEventAction()触发后转为STOP
    end note
```

**详细状态流转表**：

| 当前状态 | 触发事件 | StateAction方法 | 状态转换 | 其他操作 | 发布的事件 |
|---------|---------|----------------|---------|---------|-----------|
| `RUNNING_EXECUTION` | `PAUSE` | `WorkflowRunningStateAction.pauseEventAction()` | `RUNNING_EXECUTION` → `READY_PAUSE` | `pauseActiveTask()` 暂停活跃任务 | 无 |
| `RUNNING_EXECUTION` | `STOP` | `WorkflowRunningStateAction.stopEventAction()` | `RUNNING_EXECUTION` → `READY_STOP` | `killActiveTask()` 杀死活跃任务 | 无 |
| `RUNNING_EXECUTION` | `SUCCEED` | `WorkflowRunningStateAction.succeedEventAction()` | `RUNNING_EXECUTION` → `SUCCESS` | `workflowFinish()` 设置结束时间 | `WorkflowFinalizeLifecycleEvent` |
| `RUNNING_EXECUTION` | `FAILED` | `WorkflowRunningStateAction.failedEventAction()` | `RUNNING_EXECUTION` → `FAILURE` | `workflowFinish()` 设置结束时间 | `WorkflowFinalizeLifecycleEvent` |
| `RUNNING_EXECUTION` | 任务全部完成 | `WorkflowRunningStateAction.emitWorkflowFinishedEventIfApplicable()` | 自动检测 | 根据任务状态判断 | `WorkflowSucceedLifecycleEvent` 或 `WorkflowFailedLifecycleEvent` |
| `READY_PAUSE` | `PAUSED` | `WorkflowReadyPauseStateAction.pausedEventAction()` | `READY_PAUSE` → `PAUSE` | `workflowFinish()` 设置结束时间 | `WorkflowFinalizeLifecycleEvent` |
| `READY_PAUSE` | 任务全部完成 | `WorkflowReadyPauseStateAction.emitWorkflowFinishedEventIfApplicable()` | 自动检测 | 根据任务状态判断 | `WorkflowPausedLifecycleEvent`、`WorkflowSucceedLifecycleEvent` 或 `WorkflowFailedLifecycleEvent` |
| `READY_STOP` | `STOPPED` | `WorkflowReadyStopStateAction.stoppedEventAction()` | `READY_STOP` → `STOP` | `workflowFinish()` 设置结束时间 | `WorkflowFinalizeLifecycleEvent` |
| `READY_STOP` | 任务全部完成 | `WorkflowReadyStopStateAction.emitWorkflowFinishedEventIfApplicable()` | 自动检测 | 根据任务状态判断 | `WorkflowStoppedLifecycleEvent`、`WorkflowSucceedLifecycleEvent` 或 `WorkflowFailedLifecycleEvent` |
| `PAUSE` | `FINALIZE` | `WorkflowPausedStateAction.finalizeEventAction()` | 无（最终状态） | 清理工作流资源 | 无 |
| `STOP` | `FINALIZE` | `WorkflowStoppedStateAction.finalizeEventAction()` | 无（最终状态） | 清理工作流资源 | 无 |
| `SUCCESS` | `FINALIZE` | `WorkflowSuccessStateAction.finalizeEventAction()` | 无（最终状态） | 清理工作流资源 | 无 |
| `FAILURE` | `FINALIZE` | `WorkflowFailedStateAction.finalizeEventAction()` | 无（最终状态） | 清理工作流资源 | 无 |

**关键方法说明**：

1. **`transformWorkflowInstanceState(workflowExecutionRunnable, targetState)`**：
   - 位置：`AbstractWorkflowStateAction`
   - 功能：转换工作流状态（内存和数据库）
   - 实现：
     ```java
...
2. **`workflowFinish(workflowExecutionRunnable, workflowExecutionStatus)`**：
   - 位置：`AbstractWorkflowStateAction`
   - 功能：完成工作流，设置结束时间并转换状态
   - 实现：
     ```java
...
3. **`emitWorkflowFinishedEventIfApplicable(workflowExecutionRunnable)`**：
   - 位置：各个StateAction实现类
   - 功能：自动检测工作流是否可以完成，如果可以则发布相应的完成事件
   - 触发时机：任务完成后，在`tryToTriggerSuccessorsAfterTaskFinish()`中调用
   - 不同状态的检测逻辑：
     - `RUNNING_EXECUTION`：检测是否有失败任务，有则发布`WorkflowFailedLifecycleEvent`，否则发布`WorkflowSucceedLifecycleEvent`
     - `READY_PAUSE`：检测是否有暂停任务，有则发布`WorkflowPausedLifecycleEvent`
     - `READY_STOP`：检测是否有停止任务，有则发布`WorkflowStoppedLifecycleEvent`
...
emitWorkflowFinishedEventIfApplicable()
  ↓ (检测到所有任务成功)
发布 WorkflowSucceedLifecycleEvent
  ↓ (WorkflowSucceedLifecycleEventHandler处理)
WorkflowRunningStateAction.succeedEventAction()
  ↓
workflowFinish(SUCCESS)
  ↓
transformWorkflowInstanceState(SUCCESS)
  ↓
发布 WorkflowFinalizeLifecycleEvent
  ↓ (WorkflowFinalizeLifecycleEventHandler处理)
WorkflowSuccessStateAction.finalizeEventAction()
  ↓
清理工作流资源
```
...
WorkflowRunningStateAction.pauseEventAction()
  ↓
transformWorkflowInstanceState(READY_PAUSE)
  ↓
pauseActiveTask() (暂停所有活跃任务)
  ↓ (所有任务暂停完成)
emitWorkflowFinishedEventIfApplicable()
  ↓ (检测到有暂停任务)
发布 WorkflowPausedLifecycleEvent
  ↓ (WorkflowPausedLifecycleEventHandler处理)
WorkflowReadyPauseStateAction.pausedEventAction()
  ↓
workflowFinish(PAUSE)
  ↓
transformWorkflowInstanceState(PAUSE)
  ↓
发布 WorkflowFinalizeLifecycleEvent
  ↓ (WorkflowFinalizeLifecycleEventHandler处理)
WorkflowPausedStateAction.finalizeEventAction()
  ↓
清理工作流资源
```
...
RUNNING_EXECUTION
  ↓ (收到STOP事件)
WorkflowRunningStateAction.stopEventAction()
  ↓
transformWorkflowInstanceState(READY_STOP)
  ↓
killActiveTask() (杀死所有活跃任务)
  ↓ (所有任务停止完成)
emitWorkflowFinishedEventIfApplicable()
  ↓ (检测到有停止任务)
发布 WorkflowStoppedLifecycleEvent
  ↓ (WorkflowStoppedLifecycleEventHandler处理)
WorkflowReadyStopStateAction.stoppedEventAction()
  ↓
workflowFinish(STOP)
  ↓
transformWorkflowInstanceState(STOP)
  ↓
发布 WorkflowFinalizeLifecycleEvent
  ↓ (WorkflowFinalizeLifecycleEventHandler处理)
WorkflowStoppedStateAction.finalizeEventAction()
  ↓
清理工作流资源
```
...
完整映射路径：`WorkflowLifecycleEventType` → `WorkflowLifecycleEventHandler` → `WorkflowExecutionStatus` → `IWorkflowStateAction` → `TaskLifecycleEvent`

| WorkflowLifecycleEventType | WorkflowLifecycleEventHandler实现类 | WorkflowExecutionStatus | IWorkflowStateAction实现类 | IWorkflowStateAction方法 | 发布的任务事件 |
|---------------------------|-----------------------------------|------------------------|---------------------------|------------------------|---------------|
| START | WorkflowStartLifecycleEventHandler | RUNNING_EXECUTION(1) | WorkflowRunningStateAction | startEventAction() | **TaskStartLifecycleEvent**（实际会触发） |
| START | WorkflowStartLifecycleEventHandler | SUBMITTED_SUCCESS(0) | WorkflowSubmittedStateAction | startEventAction() | 无（只记录警告，状态不匹配会抛异常） |
| START | WorkflowStartLifecycleEventHandler | READY_PAUSE(2) | WorkflowReadyPauseStateAction | startEventAction() | 无（只记录警告，状态不匹配会抛异常） |
| START | WorkflowStartLifecycleEventHandler | PAUSE(3) | WorkflowPausedStateAction | startEventAction() | 无（只记录警告，状态不匹配会抛异常） |
| START | WorkflowStartLifecycleEventHandler | READY_STOP(4) | WorkflowReadyStopStateAction | startEventAction() | 无（只记录警告，状态不匹配会抛异常） |
| START | WorkflowStartLifecycleEventHandler | STOP(5) | WorkflowStoppedStateAction | startEventAction() | 无（只记录警告，状态不匹配会抛异常） |
| START | WorkflowStartLifecycleEventHandler | FAILURE(6) | WorkflowFailedStateAction | startEventAction() | 无（只记录警告，状态不匹配会抛异常） |
| START | WorkflowStartLifecycleEventHandler | SUCCESS(7) | WorkflowSuccessStateAction | startEventAction() | 无（只记录警告，状态不匹配会抛异常） |
| START | WorkflowStartLifecycleEventHandler | SERIAL_WAIT(14) | WorkflowSerialWaitStateAction | startEventAction() | 无（只记录警告，状态不匹配会抛异常） |
| START | WorkflowStartLifecycleEventHandler | FAILOVER(18) | WorkflowFailoverStateAction | startEventAction() | 无（只记录警告，状态不匹配会抛异常） |
| TOPOLOGY_LOGICAL_TRANSACTION_WITH_TASK_FINISH | WorkflowTopologyLogicalTransitionWithTaskFinishLifecycleEventHandler | RUNNING_EXECUTION(1) | WorkflowRunningStateAction | topologyLogicalTransitionEventAction() | **TaskStartLifecycleEvent** |
| TOPOLOGY_LOGICAL_TRANSACTION_WITH_TASK_FINISH | WorkflowTopologyLogicalTransitionWithTaskFinishLifecycleEventHandler | SUBMITTED_SUCCESS(0) | WorkflowSubmittedStateAction | topologyLogicalTransitionEventAction() | **TaskStartLifecycleEvent** |
| TOPOLOGY_LOGICAL_TRANSACTION_WITH_TASK_FINISH | WorkflowTopologyLogicalTransitionWithTaskFinishLifecycleEventHandler | READY_PAUSE(2) | WorkflowReadyPauseStateAction | topologyLogicalTransitionEventAction() | **TaskStartLifecycleEvent** |
| TOPOLOGY_LOGICAL_TRANSACTION_WITH_TASK_FINISH | WorkflowTopologyLogicalTransitionWithTaskFinishLifecycleEventHandler | PAUSE(3) | WorkflowPausedStateAction | topologyLogicalTransitionEventAction() | **TaskStartLifecycleEvent** |
| TOPOLOGY_LOGICAL_TRANSACTION_WITH_TASK_FINISH | WorkflowTopologyLogicalTransitionWithTaskFinishLifecycleEventHandler | READY_STOP(4) | WorkflowReadyStopStateAction | topologyLogicalTransitionEventAction() | **TaskStartLifecycleEvent** |
| TOPOLOGY_LOGICAL_TRANSACTION_WITH_TASK_FINISH | WorkflowTopologyLogicalTransitionWithTaskFinishLifecycleEventHandler | STOP(5) | WorkflowStoppedStateAction | topologyLogicalTransitionEventAction() | **TaskStartLifecycleEvent** |
| TOPOLOGY_LOGICAL_TRANSACTION_WITH_TASK_FINISH | WorkflowTopologyLogicalTransitionWithTaskFinishLifecycleEventHandler | FAILURE(6) | WorkflowFailedStateAction | topologyLogicalTransitionEventAction() | **TaskStartLifecycleEvent** |
| TOPOLOGY_LOGICAL_TRANSACTION_WITH_TASK_FINISH | WorkflowTopologyLogicalTransitionWithTaskFinishLifecycleEventHandler | SUCCESS(7) | WorkflowSuccessStateAction | topologyLogicalTransitionEventAction() | **TaskStartLifecycleEvent** |
| TOPOLOGY_LOGICAL_TRANSACTION_WITH_TASK_FINISH | WorkflowTopologyLogicalTransitionWithTaskFinishLifecycleEventHandler | SERIAL_WAIT(14) | WorkflowSerialWaitStateAction | topologyLogicalTransitionEventAction() | **TaskStartLifecycleEvent** |
| TOPOLOGY_LOGICAL_TRANSACTION_WITH_TASK_FINISH | WorkflowTopologyLogicalTransitionWithTaskFinishLifecycleEventHandler | FAILOVER(18) | WorkflowFailoverStateAction | topologyLogicalTransitionEventAction() | **TaskStartLifecycleEvent** |
| PAUSE | WorkflowPauseLifecycleEventHandler | RUNNING_EXECUTION(1) | WorkflowRunningStateAction | pauseEventAction() | **TaskPauseLifecycleEvent** |
| PAUSE | WorkflowPauseLifecycleEventHandler | SUBMITTED_SUCCESS(0) | WorkflowSubmittedStateAction | pauseEventAction() | **TaskPauseLifecycleEvent** |
| PAUSE | WorkflowPauseLifecycleEventHandler | READY_PAUSE(2) | WorkflowReadyPauseStateAction | pauseEventAction() | **TaskPauseLifecycleEvent** |
| PAUSE | WorkflowPauseLifecycleEventHandler | PAUSE(3) | WorkflowPausedStateAction | pauseEventAction() | **TaskPauseLifecycleEvent** |
| PAUSE | WorkflowPauseLifecycleEventHandler | READY_STOP(4) | WorkflowReadyStopStateAction | pauseEventAction() | **TaskPauseLifecycleEvent** |
| PAUSE | WorkflowPauseLifecycleEventHandler | STOP(5) | WorkflowStoppedStateAction | pauseEventAction() | **TaskPauseLifecycleEvent** |
| PAUSE | WorkflowPauseLifecycleEventHandler | FAILURE(6) | WorkflowFailedStateAction | pauseEventAction() | **TaskPauseLifecycleEvent** |
| PAUSE | WorkflowPauseLifecycleEventHandler | SUCCESS(7) | WorkflowSuccessStateAction | pauseEventAction() | **TaskPauseLifecycleEvent** |
| PAUSE | WorkflowPauseLifecycleEventHandler | SERIAL_WAIT(14) | WorkflowSerialWaitStateAction | pauseEventAction() | **TaskPauseLifecycleEvent** |
| PAUSE | WorkflowPauseLifecycleEventHandler | FAILOVER(18) | WorkflowFailoverStateAction | pauseEventAction() | **TaskPauseLifecycleEvent** |
| PAUSED | WorkflowPausedLifecycleEventHandler | PAUSE(3) | WorkflowPausedStateAction | pausedEventAction() | 无 |
| STOP | WorkflowStopLifecycleEventHandler | RUNNING_EXECUTION(1) | WorkflowRunningStateAction | stopEventAction() | **TaskKillLifecycleEvent** |
| STOP | WorkflowStopLifecycleEventHandler | SUBMITTED_SUCCESS(0) | WorkflowSubmittedStateAction | stopEventAction() | **TaskKillLifecycleEvent** |
| STOP | WorkflowStopLifecycleEventHandler | READY_PAUSE(2) | WorkflowReadyPauseStateAction | stopEventAction() | **TaskKillLifecycleEvent** |
| STOP | WorkflowStopLifecycleEventHandler | PAUSE(3) | WorkflowPausedStateAction | stopEventAction() | **TaskKillLifecycleEvent** |
| STOP | WorkflowStopLifecycleEventHandler | READY_STOP(4) | WorkflowReadyStopStateAction | stopEventAction() | **TaskKillLifecycleEvent** |
| STOP | WorkflowStopLifecycleEventHandler | STOP(5) | WorkflowStoppedStateAction | stopEventAction() | **TaskKillLifecycleEvent** |
| STOP | WorkflowStopLifecycleEventHandler | FAILURE(6) | WorkflowFailedStateAction | stopEventAction() | **TaskKillLifecycleEvent** |
| STOP | WorkflowStopLifecycleEventHandler | SUCCESS(7) | WorkflowSuccessStateAction | stopEventAction() | **TaskKillLifecycleEvent** |
| STOP | WorkflowStopLifecycleEventHandler | SERIAL_WAIT(14) | WorkflowSerialWaitStateAction | stopEventAction() | **TaskKillLifecycleEvent** |
| STOP | WorkflowStopLifecycleEventHandler | FAILOVER(18) | WorkflowFailoverStateAction | stopEventAction() | **TaskKillLifecycleEvent** |
| STOPPED | WorkflowStoppedLifecycleEventHandler | STOP(5) | WorkflowStoppedStateAction | stoppedEventAction() | 无 |
| SUCCEED | WorkflowSucceedLifecycleEventHandler | RUNNING_EXECUTION(1) | WorkflowRunningStateAction | succeedEventAction() | 无（发布WorkflowFinalizeLifecycleEvent） |
| FAILED | WorkflowFailedLifecycleEventHandler | RUNNING_EXECUTION(1) | WorkflowRunningStateAction | failedEventAction() | 无（发布WorkflowFinalizeLifecycleEvent） |
| FINALIZE | WorkflowFinalizeLifecycleEventHandler | SUCCESS(7) | WorkflowSuccessStateAction | finalizeEventAction() | 无 |
| FINALIZE | WorkflowFinalizeLifecycleEventHandler | FAILURE(6) | WorkflowFailedStateAction | finalizeEventAction() | 无 |
| FINALIZE | WorkflowFinalizeLifecycleEventHandler | STOP(5) | WorkflowStoppedStateAction | finalizeEventAction() | 无 |
| FINALIZE | WorkflowFinalizeLifecycleEventHandler | PAUSE(3) | WorkflowPausedStateAction | finalizeEventAction() | 无 |

**说明**：

1. **映射路径**：`WorkflowLifecycleEventType` → `WorkflowLifecycleEventHandler` → `WorkflowExecutionStatus` → `IWorkflowStateAction` → `TaskLifecycleEvent`

2. **关键逻辑**：
   - 每个 `WorkflowLifecycleEventType` 对应一个 `WorkflowLifecycleEventHandler` 实现类（1:1映射）
   - Handler 根据工作流当前状态（`workflowExecutionRunnable.getState()`）从 `WorkflowStateActionFactory.getAction(status)` 获取对应的 `IWorkflowStateAction` 实现类
   - Handler 根据事件类型调用 `IWorkflowStateAction` 的相应方法
   - `IWorkflowStateAction` 方法执行后，可能会发布任务生命周期事件（如 `TaskStartLifecycleEvent`、`TaskPauseLifecycleEvent`、`TaskKillLifecycleEvent` 等）

3. **<span style="color: #8B0000;">重要说明：assembleWorkflowInstance 实际设置的状态</span>**：
   
   虽然映射表中列出了所有可能的状态组合，但在实际代码中，`assembleWorkflowInstance()` 方法设置的状态是有限的：
   
   | CommandHandler | 设置的状态 | 说明 |
   |---------------|-----------|------|
   | `RunWorkflowCommandHandler` | `RUNNING_EXECUTION(1)` | 启动工作流 |
   | `ReRunWorkflowCommandHandler` | `RUNNING_EXECUTION(1)` | 重跑工作流 |
   | `ScheduleWorkflowCommandHandler` | `RUNNING_EXECUTION(1)` | 调度触发（继承自RunWorkflowCommandHandler） |
   | `BackfillWorkflowCommandHandler` | `RUNNING_EXECUTION(1)` | 补数（继承自RunWorkflowCommandHandler） |
   | `RecoverFailureTaskCommandHandler` | `RUNNING_EXECUTION(1)` | 恢复失败任务 |
   | `RecoverSuspendWorkflowCommandHandler` | `RUNNING_EXECUTION(1)` | 恢复暂停（继承自RecoverFailureTaskCommandHandler） |
   | `WorkflowFailoverCommandHandler` | **从参数获取**（可能是任意状态） | 故障转移，状态从`WorkflowFailoverCommandParam.getWorkflowExecutionStatus()`获取 |
   
   **结论**：
   - **大部分情况下**，`assembleWorkflowInstance` 时设置的状态都是 `RUNNING_EXECUTION(1)`
   - 只有 `WorkflowFailoverCommandHandler` 可能设置为其他状态（从命令参数中获取）
   - 因此，**START事件在实际使用中，主要对应 `RUNNING_EXECUTION` 状态**

4. **为什么映射表中START事件会对应所有状态？**
   
   这是**状态机模式的设计**：
   - 每个 `IWorkflowStateAction` 实现类都实现了接口的所有方法（包括 `startEventAction()`）
   - 理论上，任何状态都可以接收START事件
   - 但不同状态的StateAction对START事件的处理逻辑不同：
     - `WorkflowRunningStateAction.startEventAction()`: **会触发任务**（调用`triggerTasks()`）
     - `WorkflowSubmittedStateAction.startEventAction()`: **只记录警告**（调用`logWarningIfCannotDoAction()`）
     - `WorkflowPausedStateAction.startEventAction()`: **只记录警告**（调用`logWarningIfCannotDoAction()`）
     - `WorkflowStoppedStateAction.startEventAction()`: **只记录警告**（调用`logWarningIfCannotDoAction()`）
     - 等等
   
   **状态检查机制**：
   - 每个StateAction的`startEventAction`方法都会先调用`throwExceptionIfStateIsNotMatch(workflowExecutionRunnable)`
   - 如果工作流的实际状态与StateAction匹配的状态不一致，会抛出`IllegalStateException`
   - 这意味着：虽然理论上任何状态都可以接收START事件，但**只有状态匹配时才会真正处理，否则会抛出异常**
   
   **实际使用场景**：
   - 在`CommandEngine.bootstrapWorkflowExecutionRunnable()`中发布START事件时，工作流的状态是在`assembleWorkflowInstance`时设置的
   - 大部分情况下是`RUNNING_EXECUTION`，所以会调用`WorkflowRunningStateAction.startEventAction()`，正常触发任务
   - 如果状态不匹配（比如工作流处于PAUSE状态时收到START事件），会抛出异常或只记录警告

3. **状态到StateAction的映射**（10种状态对应10个实现类）：
   - `SUBMITTED_SUCCESS(0)` → `WorkflowSubmittedStateAction`
   - `RUNNING_EXECUTION(1)` → `WorkflowRunningStateAction`
   - `READY_PAUSE(2)` → `WorkflowReadyPauseStateAction`
   - `PAUSE(3)` → `WorkflowPausedStateAction`
   - `READY_STOP(4)` → `WorkflowReadyStopStateAction`
   - `STOP(5)` → `WorkflowStoppedStateAction`
   - `FAILURE(6)` → `WorkflowFailedStateAction`
   - `SUCCESS(7)` → `WorkflowSuccessStateAction`
   - `SERIAL_WAIT(14)` → `WorkflowSerialWaitStateAction`
   - `FAILOVER(18)` → `WorkflowFailoverStateAction`

4. **事件类型到方法的映射**（9种事件类型对应9个方法）：
     - `START` → `startEventAction()`
     - `TOPOLOGY_LOGICAL_TRANSACTION_WITH_TASK_FINISH` → `topologyLogicalTransitionEventAction()`
     - `PAUSE` → `pauseEventAction()`
     - `PAUSED` → `pausedEventAction()`
     - `STOP` → `stopEventAction()`
     - `STOPPED` → `stoppedEventAction()`
     - `SUCCEED` → `succeedEventAction()`
     - `FAILED` → `failedEventAction()`
     - `FINALIZE` → `finalizeEventAction()`

5. **任务事件说明**：
   - `TaskStartLifecycleEvent`：由 `triggerTasks()` 方法发布，用于触发任务执行
   - `TaskPauseLifecycleEvent`：由 `pauseActiveTask()` 方法发布，用于暂停活跃任务
   - `TaskKillLifecycleEvent`：由 `killActiveTask()` 方法发布，用于杀死活跃任务
   - 某些事件（如 `PAUSED`、`STOPPED`、`SUCCEED`、`FAILED`、`FINALIZE`）不会发布任务事件，而是处理工作流状态转换或清理工作
