...
#### 9.2.2.1 WorkflowLifecycleEventType 与 WorkflowExecutionStatus 关系图

```mermaid
graph TB
    subgraph "WorkflowExecutionStatus<br/>工作流执行状态"
        direction TB
        S1[SUBMITTED_SUCCESS<br/>0<br/>已提交成功]
        S2[RUNNING_EXECUTION<br/>1<br/>运行中]
        S3[READY_PAUSE<br/>2<br/>准备暂停]
        S4[PAUSE<br/>3<br/>已暂停]
        S5[READY_STOP<br/>4<br/>准备停止]
        S6[STOP<br/>5<br/>已停止]
        S7[FAILURE<br/>6<br/>失败]
        S8[SUCCESS<br/>7<br/>成功]
        S9[SERIAL_WAIT<br/>14<br/>串行等待]
        S10[FAILOVER<br/>18<br/>故障转移]
    end
    
    subgraph "WorkflowStateActionFactory<br/>状态动作工厂"
        direction TB
        WSAF[根据WorkflowExecutionStatus<br/>获取IWorkflowStateAction<br/>getAction_status]
    end
    
    subgraph "IWorkflowStateAction实现类<br/>状态动作实现"
        direction TB
        A1[WorkflowSubmittedStateAction<br/>匹配SUBMITTED_SUCCESS]
        A2[WorkflowRunningStateAction<br/>匹配RUNNING_EXECUTION]
        A3[WorkflowReadyPauseStateAction<br/>匹配READY_PAUSE]
        A4[WorkflowPausedStateAction<br/>匹配PAUSE]
        A5[WorkflowReadyStopStateAction<br/>匹配READY_STOP]
        A6[WorkflowStoppedStateAction<br/>匹配STOP]
        A7[WorkflowFailedStateAction<br/>匹配FAILURE]
        A8[WorkflowSuccessStateAction<br/>匹配SUCCESS]
        A9[WorkflowSerialWaitStateAction<br/>匹配SERIAL_WAIT]
        A10[WorkflowFailoverStateAction<br/>匹配FAILOVER]
    end
    
    subgraph "WorkflowLifecycleEventType<br/>工作流生命周期事件类型"
        direction TB
        E1[START<br/>启动事件]
        E2[TOPOLOGY_LOGICAL_TRANSACTION_WITH_TASK_FINISH<br/>拓扑逻辑转换事件]
        E3[PAUSE<br/>暂停事件]
        E4[PAUSED<br/>已暂停事件]
        E5[STOP<br/>停止事件]
        E6[STOPPED<br/>已停止事件]
    ...
    subgraph "IWorkflowStateAction方法<br/>状态动作方法"
        direction TB
        M1[startEventAction<br/>处理启动事件]
        M2[topologyLogicalTransitionEventAction<br/>处理拓扑转换事件]
        M3[pauseEventAction<br/>处理暂停事件]
        M4[pausedEventAction<br/>处理已暂停事件]
        M5[stopEventAction<br/>处理停止事件]
        M6[stoppedEventAction<br/>处理已停止事件]
        M7[succeedEventAction<br/>处理成功事件]
        M8[failedEventAction<br/>处理失败事件]
        M9[finalizeEventAction<br/>处理完成事件]
    end
    ...
    style E5 fill:#f1f8e9
    style E6 fill:#f1f8e9
    style E7 fill:#f1f8e9
    style E8 fill:#f1f8e9
    ...
    style M9 fill:#fff9c4
```

**关系说明**：

1. **WorkflowExecutionStatus 与 IWorkflowStateAction 的映射关系**：
   - 每个 `WorkflowExecutionStatus` 状态都有唯一对应的 `IWorkflowStateAction` 实现类
   - 通过 `WorkflowStateActionFactory.getAction(WorkflowExecutionStatus)` 方法获取对应的状态动作
   - 映射关系是一对一的，每个状态对应一个实现类

2. **IWorkflowStateAction 实现类的职责**：
   - 每个实现类都实现了 `IWorkflowStateAction` 接口的所有9个方法
   - 每个实现类通过 `matchState()` 方法声明自己匹配的状态
   - 在方法实现中，根据当前状态和事件类型决定是否执行相应逻辑

3. **WorkflowLifecycleEventType 与 IWorkflowStateAction 方法的映射关系**：
   - 每个 `WorkflowLifecycleEventType` 事件类型对应 `IWorkflowStateAction` 接口中的一个方法
   - 事件类型与方法名称的对应关系：
...
4. **处理流程**：
   - Handler 获取工作流当前状态（`WorkflowExecutionStatus`）
   - Handler 根据状态从 `WorkflowStateActionFactory` 获取对应的 `IWorkflowStateAction`
   - Handler 根据事件类型（`WorkflowLifecycleEventType`）调用 `IWorkflowStateAction` 的相应方法
   - `IWorkflowStateAction` 实现类根据当前状态和事件类型执行相应的业务逻辑
