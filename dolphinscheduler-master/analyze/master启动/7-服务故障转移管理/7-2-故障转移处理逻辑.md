...
**DelayQueue.take() 详细原理分析**

`DelayQueue.take()` 是一个阻塞方法，它会一直等待直到有元素到期才返回。下面详细分析其工作原理：
...
public E take() throws InterruptedException {
    ...
    try {
        for (;;) {  // 无限循环，直到获取到可用元素
            E first = q.peek();  // 查看堆顶元素（不删除）
            if (first == null) {
                // 【场景1】队列为空：线程在 available 条件变量上等待
                available.await();  // 阻塞等待，直到有其他线程调用 signal()
            } else {
                long delay = first.getDelay(NANOSECONDS);  // 获取剩余延迟时间
                if (delay <= 0) {
                    // 【场景2】元素已到期：立即返回
                    return q.poll();  // 移除并返回堆顶元素
                }
                // 【场景3】元素未到期：释放引用，避免内存泄漏
                first = null;
                // 如果有其他线程也在等待（leader 不为 null），让其他线程等待
                if (leader != null) {
                    available.await();  // 等待
                } else {
                    // 当前线程成为 leader，等待剩余延迟时间
                    Thread thisThread = Thread.currentThread();
                    leader = thisThread;
                    try {
                        // 使用 awaitNanos() 阻塞等待剩余延迟时间
                        available.awaitNanos(delay);  // 纳秒级精确等待
                    } finally {
        ...
        // 如果队列不为空且 leader 为 null，唤醒其他等待线程
...
| 场景 | 条件 | 阻塞方式 | 唤醒时机 |
|------|------|---------|---------|
| **场景1：队列为空** | `q.peek() == null` | `available.await()` | 其他线程调用 `offer/add/put` 添加元素时，会调用 `available.signal()` 唤醒 |
| **场景2：元素已到期** | `delay <= 0` | 不阻塞，立即返回 | 无 |
| **场景3：元素未到期** | `delay > 0` | `available.awaitNanos(delay)` | 等待时间到期自动唤醒，或新元素比当前堆顶更早到期时被唤醒 |
...
**唤醒流程**：
1. 线程A调用 `take()`，队列为空，在 `available.await()` 上阻塞
2. 线程B调用 `offer(event)`，将元素添加到队列
3. 如果新元素成为堆顶（`q.peek() == e`），调用 `available.signal()` 唤醒线程A
4. 线程A被唤醒，重新检查队列，获取到元素并返回

**2.2 元素到期时的自动唤醒**

```java
// 当堆顶元素未到期时
long delay = first.getDelay(NANOSECONDS);  // 假设返回 3000000000 纳秒（3秒）
if (delay > 0) {
    available.awaitNanos(delay);  // 阻塞等待 3 秒
    // 3 秒后自动唤醒，重新检查堆顶元素是否到期
}
```

**2.3 新元素比当前堆顶更早到期时的唤醒**

```java
// 假设当前堆顶元素还有 10 秒到期，线程A正在 awaitNanos(10s) 等待
// 此时线程B添加了一个 2 秒后到期的元素
public boolean offer(E e) {
    lock.lock();
    try {
        q.offer(e);  // 添加到队列
        if (q.peek() == e) {  // 新元素成为堆顶（比原来的更早到期）
            leader = null;
            available.signal();  // 【关键】唤醒正在等待的线程A
        }
        return true;
    } finally {
        lock.unlock();
...
**唤醒流程**：
1. 线程A调用 `take()`，堆顶元素还有 10 秒到期，调用 `awaitNanos(10s)` 阻塞
2. 线程B调用 `offer(event)`，添加了一个 2 秒后到期的元素
3. 新元素比原来的堆顶更早到期，成为新的堆顶（`q.peek() == e`）
4. 调用 `available.signal()` 唤醒线程A
5. 线程A被唤醒，重新检查堆顶元素，发现已到期（或更接近到期），立即返回

**3. Leader-Follower 模式**

`DelayQueue` 使用 **Leader-Follower 模式** 优化性能：

```java
private Thread leader = null;  // Leader 线程，只有一个线程在等待堆顶元素到期
```

**工作原理**：
- **Leader 线程**：正在等待堆顶元素到期的线程，使用 `awaitNanos(delay)` 精确等待
- **Follower 线程**：其他等待的线程，使用 `await()` 无限期等待

**优势**：
- 避免不必要的唤醒：只有 Leader 线程在精确等待，其他线程等待 Leader 唤醒
- 减少上下文切换：避免多个线程同时等待同一个元素到期

**示例**：
```java
// 线程A：调用 take()，堆顶元素还有 5 秒到期
leader = Thread-A;
available.awaitNanos(5_000_000_000);  // 精确等待 5 秒

// 线程B：也调用 take()，发现 leader != null
if (leader != null) {
    available.await();  // 无限期等待，直到被 Leader 唤醒
}

// 5 秒后，线程A被唤醒，获取元素
// 线程A退出时，如果队列还有元素，会唤醒线程B
if (leader == null && q.peek() != null) {
    available.signal();  // 唤醒线程B
}
```
...
public E take() throws InterruptedException {
    ...
    try {
        for (;;) {
            // ...
            if (first == null) {
                available.await();  // 如果线程被中断，会抛出 InterruptedException
            } else {
                available.awaitNanos(delay);  // 如果线程被中断，会抛出 InterruptedException
            }
        }
    } finally {
        lock.unlock();
...
**中断行为**：
- 如果线程在 `await()` 或 `awaitNanos()` 中被中断，会立即抛出 `InterruptedException`
- 调用者可以捕获异常并处理中断逻辑（如优雅关闭）

**5. 完整执行流程图**

```mermaid
flowchart TD
    Start([调用 take]) --> Lock[获取锁 lock.lockInterruptibly]
    Lock --> Loop{进入循环 for}
    Loop --> Peek[查看堆顶元素 q.peek]
    Peek --> CheckNull{队列是否为空?}
    
    CheckNull -->|是| Wait1[available.await<br/>阻塞等待]
    Wait1 --> Signal1{收到 signal?}
    Signal1 -->|是| Loop
    Signal1 -->|中断| Interrupt1[抛出 InterruptedException]
    
    CheckNull -->|否| GetDelay[调用 getDelay<br/>获取剩余延迟时间]
    GetDelay --> CheckDelay{delay <= 0?}
    
    CheckDelay -->|是| Poll[移除并返回堆顶元素 q.poll]
    Poll --> Return1[返回元素]
    
    CheckDelay -->|否| CheckLeader{leader 是否为 null?}
    CheckLeader -->|否| Wait2[available.await<br/>等待 Leader 完成]
    Wait2 --> Signal2{收到 signal?}
    Signal2 -->|是| Loop
    Signal2 -->|中断| Interrupt2[抛出 InterruptedException]
    
    CheckLeader -->|是| SetLeader[设置 leader = 当前线程]
    SetLeader --> WaitNanos[available.awaitNanos delay<br/>精确等待剩余时间]
    WaitNanos --> Wake1{等待到期?}
    Wake1 -->|是| ResetLeader[重置 leader = null]
    Wake1 -->|中断| Interrupt3[抛出 InterruptedException]
    ResetLeader --> Loop
    
    Return1 --> Unlock[释放锁 lock.unlock]
    Unlock --> WakeOthers{队列还有元素<br/>且 leader == null?}
    WakeOthers -->|是| Signal3[available.signal<br/>唤醒其他等待线程]
    WakeOthers -->|否| End([方法结束])
    Signal3 --> End
...
| 设计点 | 说明 |
|--------|------|
| **阻塞等待** | 队列为空或元素未到期时，线程会阻塞，避免忙等待（busy-waiting） |
| **Condition 机制** | 使用 `ReentrantLock` 和 `Condition` 实现高效的线程等待和唤醒 |
| **Leader-Follower** | 只有一个线程（Leader）精确等待，其他线程等待 Leader 唤醒，减少上下文切换 |
| **自动唤醒** | 使用 `awaitNanos(delay)` 精确等待到期时间，自动唤醒，无需轮询 |
| **优先级队列** | 内部使用 `PriorityQueue` 按过期时间排序，确保最早到期的元素在堆顶 |
| **中断支持** | 支持线程中断，可以优雅地停止等待 |
| **线程安全** | 所有操作都在锁保护下进行，保证线程安全 |

**7. 应用场景**

在 DolphinScheduler 中，`SystemEventBus.take()` 用于：

1. **WorkerFailoverEvent**：延迟 30 秒后处理 Worker 故障转移
2. **MasterFailoverEvent**：延迟 30 秒后处理 Master 故障转移
3. **GlobalMasterFailoverEvent**：系统启动时处理全局 Master 故障转移

**示例**：
```java
// SystemEventBusFireWorker.run()
while (flag) {
    // 阻塞等待，直到有事件到期
    AbstractSystemEvent event = systemEventBus.take();  // 队列为空或事件未到期时会阻塞
    // 处理事件
...
**优势**：
- **节省 CPU**：线程阻塞等待，不会占用 CPU 资源
- **精确延迟**：使用纳秒级时间计算，延迟精度高
- **自动唤醒**：无需轮询，事件到期自动唤醒线程
