...
graph TB
    subgraph "继承复用层次"
        ICH[ICommandHandler<br/>接口层]
        ACH[AbstractCommandHandler<br/>抽象层<br/>提供通用逻辑]
        RWCH[RunWorkflowCommandHandler<br/>基础实现层<br/>新工作流启动]
        RFCH[RecoverFailureTaskCommandHandler<br/>基础实现层<br/>失败任务恢复]
        WFCH[WorkflowFailoverCommandHandler<br/>基础实现层<br/>故障转移]
        
        RRWCH[ReRunWorkflowCommandHandler<br/>复用层<br/>重跑工作流]
        SWCH[ScheduleWorkflowCommandHandler<br/>复用层<br/>调度触发]
        BWCH[BackfillWorkflowCommandHandler<br/>复用层<br/>补数]
        RSWCH[RecoverSuspendWorkflowCommandHandler<br/>复用层<br/>恢复暂停]
    end
    ...
    style ACH fill:#ffcc99
    style RWCH fill:#99ccff
    style RFCH fill:#99ccff
    style WFCH fill:#99ccff
    style RRWCH fill:#99ff99
    style SWCH fill:#99ff99
    style BWCH fill:#99ff99
    style RSWCH fill:#99ff99
```
...
这种设计使得系统具有良好的可扩展性、可维护性和可靠性，能够支持大规模的工作流调度场景。
